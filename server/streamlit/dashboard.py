    ...
    with st.spinner("🧠 백엔드 AI 엔진에서 진짜 감정 가중치를 계산하고 있습니다..."):
        try:
            ...
            if response.status_code == 200:
                ...
                bad_safe_zone = 0.1 #bad가 0.1 이하면 good 판정(감정을 분류하진 못했지만 나쁜말은 아님)
                ...
                with th_col2:
                    ...
                    elif bad_score <= bad_safe_zone:
                        st.markdown(f"🔴 **판정 결과:** 기준치를 넘기진 못했지만 bad가 ({bad_safe_zone*100:.0f}%) 이하이므로 **GOOD**")
                    else:
